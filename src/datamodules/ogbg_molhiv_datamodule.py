...
class OGBGMolhiv(LightningDataModule):
    def __init__(
        self,
        data_dir: str = "data/",
        batch_size: int = 64,
    ...
    def prepare_data(self):
        ...
        PygGraphPropPredDataset(
            name="ogbg-molhiv", root=self.data_dir, pre_transform=self.pre_transform
        )

    def setup(self, stage: Optional[str] = None):
        """Load data. Set variables: self.data_train, self.data_val, self.data_test."""
        dataset = PygGraphPropPredDataset(
            name="ogbg-molhiv", root=self.data_dir, transform=self.transform
        )
