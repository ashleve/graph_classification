...
def init_model(model_config: dict, optimizer_config: dict) -> pl.LightningModule:
    ...
    SimplePixelMnistClassifier = load_class(model_class)
    assert issubclass(SimplePixelMnistClassifier, pl.LightningModule), \
        f"Specified model class `{model_class}` is not a subclass of `LightningModule`."
    return SimplePixelMnistClassifier(model_config=model_config, optimizer_config=optimizer_config)
